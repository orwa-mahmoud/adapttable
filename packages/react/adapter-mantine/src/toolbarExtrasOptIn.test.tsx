/**
 * "Omitting a prop renders nothing" — proved for Mantine's toolbar extras.
 *
...
describe("Mantine toolbar extras are opt-in", () => {
  it("draws no undo/redo without BOTH handlers", () => {
    const only = renderMantine(
    ...
    const neither = renderMantine(<UndoRedoButtons {...base} />);
  ...
  it("draws no export button without a handler", () => {
    const { container } = renderMantine(<ExportCsvButton {...base} />);
