...
head:
  ...
  - tag: script
    ...
    content: '{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"AdaptTable","item":"https://adapttable.orwamahmoud.com/"},{"@type":"ListItem","position":2,"name":"AdaptTable
      API reference — every
      export","item":"https://adapttable.orwamahmoud.com/v2/react/api/"}]}'
  - tag: meta
    attrs:
      property: og:image
      content: https://adapttable.orwamahmoud.com/og/api.png
  - tag: meta
    attrs:
      name: twitter:image
      content: https://adapttable.orwamahmoud.com/og/api.png
...
| `features`       | `readonly TableFeature<TRow>[]`                        | —        | Compose opt-in features from `@adapttable/<kit>/<feature>` subpaths. Identical to the enabling props, which are deprecated until v3. See [feature composition](/v2/react/features/). |
...
| Prop                      | Type                                | Default | Description                                                                                                                                                                                                                                          |
| ------------------------- | ----------------------------------- | ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `columns`                 | `ColumnInput<TRow>[]`               | —       | Leaf `ColumnDef`s and optional `ColumnGroupDef` parents — see [ColumnDef](#columndef) and [column groups](/v2/react/column-groups/).                                                                                                                       |
...
| `collapsibleColumnGroups` | `boolean`                           | `false` | Group headers gain a collapse toggle. Each group decides what remains: an arrow stub, `collapsedKey`, or `collapsedRender`. State lives on `columnLayout.collapsedGroups` and the URL (`colGroupCollapse`). See [column groups](/v2/react/column-groups/). |
...
| `fitColumns`              | `boolean`                           | `false` | Columns share the container's width: a `flex` column takes that share, a `width` column keeps it, the rest divide what is left; `minWidth` / `maxWidth` hold either way. See [column management](/v2/react/column-management/).                            |
...
| `renderCard`                | `(row, card) => ReactNode`                                         | —                     | Replace a mobile card's body; the shell keeps selection, actions and expansion. See [mobile](/v2/react/mobile/).                                                                                                                                                                                                                                                                       |
| `mobileBreakpoint`          | `number`                                                           | `768`                 | Width (px) at or below which the card layout takes over. See [mobile](/v2/react/mobile/).                                                                                                                                                                                                                                                                                              |
...
| `commandPalette`            | `boolean \| CommandPaletteOptions`                                 | `false`               | Cmd/Ctrl+K palette of table actions; pass `{ commands, shortcuts }` to add your own. See [customization](/v2/react/customization/#command-palette).                                                                                                                                                                                                                                    |
| `contextMenu`               | `boolean \| ContextMenuOptions<TRow>`                              | `false`               | Right-click menus for headers, rows and cells; `{ items }` appends your own behind a divider. See [customization](/v2/react/customization/#context-menus).                                                                                                                                                                                                                             |
| `sidePanel`                 | `SidePanelOptions`                                                 | —                     | Controlled settings panel docked beside the table (`panels`, `open`, `onOpenChange`, `side`). See [customization](/v2/react/customization/#side-panel).                                                                                                                                                                                                                                |
...
| `isCellFlashing`            | `(rowId: string, columnKey: string) => boolean`                    | —                     | Mark cells a patch just changed (`data-flash` on the cell and on the matching card value). Pair with `useChangedCellFlash` from `@adapttable/core/stream`. Omit and nothing is marked. See [realtime](/v2/react/realtime/).                                                                                                                                                            |
| `rowStyle`                  | `(row: TRow, index: number) => CSSProperties \| undefined`         | —                     | Conditional per-row inline style, on desktop rows and mobile cards alike. See [row styling and heights](/v2/react/row-styling/).                                                                                                                                                                                                                                                       |
| `rowHeight`                 | `number \| ((row: TRow, index: number) => number)`                 | —                     | Row height in px. Sets the row and the virtualizer's `estimateSize`. `measureElement` still reports what the browser laid out. See [row styling and heights](/v2/react/row-styling/).                                                                                                                                                                                                  |
...
| `defaultExpandedRowIds`     | `readonly string[]`                                                | —                     | Row ids whose detail panel starts open. Uncontrolled initial state. See [row expansion](/v2/react/row-expansion/).                                                                                                                                                                                                                                                                     |
| `nestedTable`               | `NestedTableFor<TRow>`                                             | —                     | A real table under a row: return `{ label, table(defaults) }`, or `undefined` for a row without one. See [tree data](/v2/react/tree-data/).                                                                                                                                                                                                                                            |
| `getChildren`               | `(row: TRow) => readonly TRow[] \| undefined`                      | —                     | Tree data from nested rows. Its presence (or `getParentId`) arms the tree. See [tree data](/v2/react/tree-data/).                                                                                                                                                                                                                                                                      |
...
| `editHistory`               | `boolean \| { depth?: number }`                                    | —                     | Undo and redo through `onCellEdit` (50 gestures by default). See [cell editing](/v2/react/cell-editing/).                                                                                                                                                                                                                                                                              |
| `cellNavigation`            | `boolean`                                                          | `false`               | Keyboard cell navigation: one tab stop, `role="grid"`, arrow / Home / End / Page keys. Desktop table only. See [cell navigation](/v2/react/cell-navigation/).                                                                                                                                                                                                                          |
...
| `onRowReorder`              | `RowReorderHandler<TRow>`                                          | —                     | Drag handle in a reserved column. `from` / `to` are dataset-relative; the table never mutates rows. Keyboard: Space lifts, arrows move, Space drops, Escape cancels. Grouping or a tree refuses with a `devWarn`. See [row reordering](/v2/react/row-reordering/).                                                                                                                     |
| `pinnedRowIds`              | `RowPinState`                                                      | —                     | Controlled `{ top, bottom }` row-id lists. Pinned rows leave the virtual window and stick above or below the scroll box. See [row pinning](/v2/react/row-pinning/).                                                                                                                                                                                                                    |
...
| `getCellSpan`               | `GetCellSpan<TRow>`                                                | —                     | Per-cell `{ colSpan, rowSpan }`. Covered cells are omitted from the row's cell list. Receives `sectionRows` / `sectionRowIndex` in visual body order (pinned top, scroll, pinned bottom) so a consecutive merge stays one cell across a pin. Clips at pin boundaries and the column window. Mobile cards ignore geometry. See [row and column spanning](/v2/react/row-spanning/).      |
| `cellSpanAppearance`        | `"merged" \| "plain"`                                              | `"merged"`            | How a spanned cell is painted. `"merged"` is centered content and one fill; `"plain"` is geometry only. See [row and column spanning](/v2/react/row-spanning/).                                                                                                                                                                                                                        |
| `extraRows`                 | `readonly ExtraRow[]`                                              | —                     | Host-injected separator and full-width slots, spliced in by `beforeRowId`. Omit and nothing is inserted. See [full-width and separator rows](/v2/react/full-width-rows/).                                                                                                                                                                                                              |
...
[URL-synced state](/v2/react/url-state/). `savedViews` is declared on both the
...
bundle saving yet**. See [feature composition](/v2/react/features/).
...
| `group`              | `string \| readonly string[]`                                                        | —             | Presentational header group shortcut. A string is one level; a path stacks rows. Contiguous same-path columns merge; a reorder splits the group. Prefer a `ColumnGroupDef` with `children` when the group has collapse options. See [column groups](/v2/react/column-groups/). |
...
| `parseValue`         | `(draft: string, row: TRow) => unknown`                                              | —             | Turns an edited draft into the value committed to `onCellEdit`. See [cell editing](/v2/react/cell-editing/).                                                                                                                                                                   |
| `validate`           | `(value: unknown, row: TRow) => string \| undefined \| Promise<string \| undefined>` | —             | Judges one value before it commits; a message keeps the editor open. See [cell editing](/v2/react/cell-editing/).                                                                                                                                                              |
| `sortable`           | `boolean`                                                                            | `false`       | Enable sorting for this column.                                                                                                                                                                                                                                          |
| `colSpan`            | `number \| ((row: TRow) => number)`                                                  | `1`           | Columns this cell covers. Covered neighbours are omitted. See [row and column spanning](/v2/react/row-spanning/).                                                                                                                                                              |
...
| `flex`               | `number`                                                                             | —             | This column's share of the leftover width under `fitColumns` (`flex: 2` takes twice `flex: 1`). See [column management](/v2/react/column-management/).                                                                                                                         |
...
| `responsivePriority` | `number`                                                                             | —             | How readily this column is given up when the table is too narrow. Priority 1 is kept longest; omitting it means never dropped. See [mobile](/v2/react/mobile/).                                                                                                                |
...
[column groups](/v2/react/column-groups/).
...
* `computed({ key, deps, value, format })` — a derived column, cached per row,
  ...
  [columns](/v2/react/columns/).
* `aggregate(spec, options)` — builds the `summaryRow` / `groupAggregates`
  mapper. See [row grouping](/v2/react/row-grouping/).
* `applyRowPatches(rows, patches, getRowId)` with `insertRow` / `updateRow` /
  ...
  (spreading the array drops it). See [cell editing](/v2/react/cell-editing/).
...
See [cell editing](/v2/react/cell-editing/).
...
See [row grouping](/v2/react/row-grouping/).
...
[customization](/v2/react/customization/#export).
...
[accessibility](/v2/react/accessibility/).
...
[customization](/v2/react/customization/#spreadsheet-xlsx-export).
...
[PDF export and print layout](/v2/react/export-pdf/).
...
[sparkline columns](/v2/react/sparkline/).
...
[cell navigation](/v2/react/cell-navigation/).
...
column's name. See [cell navigation](/v2/react/cell-navigation/).
...
`onCellCut`. See [cell navigation](/v2/react/cell-navigation/).
...
`editHistory`. See [cell editing](/v2/react/cell-editing/).
...
[row grouping](/v2/react/row-grouping/).
...
`shell.autoSizeColumns`. See [column management](/v2/react/column-management/).
...
[virtualization](/v2/react/virtualization/).
...
parent contributes). See [tree data](/v2/react/tree-data/).
See [tree data](/v2/react/tree-data/).
...
session only. See [saved views](/v2/react/saved-views/).
...
[saved views](/v2/react/saved-views/).
...
[server queries](/v2/server-queries/).
...
[server queries](/v2/server-queries/#decoding-a-parameter-yourself).
...
`PIVOT_GRAND_TOTAL_KEY`. See [pivot tables](/v2/react/pivot/).
...
the panel and the column header share. See [pivot tables](/v2/react/pivot/).
...
parameter. See [URL state](/v2/react/url-state/).
...
the panel needs no pointer. See [pivot tables](/v2/react/pivot/).
...
See [pivot tables](/v2/react/pivot/#rendering-it-with-your-kit).
...
[customization](/v2/react/customization/).
...
[customization](/v2/react/customization/#command-palette).
...
[customization](/v2/react/customization/#context-menus).
...
[customization](/v2/react/customization/#side-panel).
...
[customization](/v2/react/customization/#toolbar-and-status-bar).
...
a value the user cannot see. See [columns](/v2/react/columns/).

**Odds and ends.** `ComputedColumnSpec` is the declaration
[`computed`](/v2/react/columns/) takes. `TableQueryKeyOptions` options the cache-key
...
| `aggregate`                                                                                                                                                                                                                                                                                                              | Builds a `summaryRow` / `groupAggregates` mapper from a declaration — see [row grouping](/v2/react/row-grouping/).                                                                                                                                                                                                                                                                                                                                                                                         |
...
| `ColumnGroupDef<TRow>` / `ColumnInput<TRow>`                                                                                                                                                                                                                                                                             | A parent header with `children`, or a leaf-or-parent union. See [column groups](/v2/react/column-groups/).                                                                                                                                                                                                                                                                                                                                                                                                 |
...
| `GetCellSpan` / `GetCellSpanArgs` / `BodyCell` / `CellSpanRequest` / `CellSpanAppearance`                                                                                                                                                                                                                                | Span callback / `{ row, column, rowIndex, columnIndex, sectionRows, sectionRowIndex }` / one rendered body cell / `{ colSpan?, rowSpan? }` / `"merged" \| "plain"`. See [row and column spanning](/v2/react/row-spanning/).                                                                                                                                                                                                                                                                                |
| `ExtraRow` / `ExtraEntry` / `ExtraRowKind`                                                                                                                                                                                                                                                                               | Host-injected slot / the spliced entry / `"separator" \| "fullWidth"`. See [full-width and separator rows](/v2/react/full-width-rows/).                                                                                                                                                                                                                                                                                                                                                                    |
...
| `RowStyle` / `RowHeight`                                                                                                                                                                                                                                                                                                 | Per-row style callback / a number or `(row, index) => number`. See [row styling and heights](/v2/react/row-styling/).                                                                                                                                                                                                                                                                                                                                                                                      |
...
| `RowPinState` / `RowPinSide`                                                                                                                                                                                                                                                                                             | `{ top, bottom }` id lists / `"top" \| "bottom"`. See [row pinning](/v2/react/row-pinning/).                                                                                                                                                                                                                                                                                                                                                                                                               |
| `RowPinningState` / `RowPinLabels`                                                                                                                                                                                                                                                                                       | Headless pin state and the three action strings.                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| `ExportCsvProp`                                                                                                                                                                                                                                                                                                          | The `exportCsv` prop as declared: `true`, an `ExportCsvOptions`, or absent. See [export](/v2/react/customization/#export).                                                                                                                                                                                                                                                                                                                                                                                 |
| `WidthColumn`                                                                                                                                                                                                                                                                                                            | A column reduced to what width resolution reads — its `key` and declared `width`.                                                                                                                                                                                                                                                                                                                                                                                                                    |
| `GroupedHeaderAlign`                                                                                                                                                                                                                                                                                                     | Alignment of a spanning group header: `"start"`, `"center"` or `"end"`. See [column groups](/v2/react/column-groups/).                                                                                                                                                                                                                                                                                                                                                                                     |
| `BuildGroupedFlatModelOptions`                                                                                                                                                                                                                                                                                           | What `buildGroupedFlatModel` flattens — `rows`, `groupBy`, `columns`, `getRowId` and the collapse/aggregate options. See [row grouping](/v2/react/row-grouping/).                                                                                                                                                                                                                                                                                                                                          |
...
| `SESSION_ATTR`                                                                                                                                                                                                                                                                                                           | `data-adapttable-header-filter` — the attribute tying a header filter's trigger to its overlay, so one editing session is identifiable across both. See [filtering](/v2/react/filtering/).                                                                                                                                                                                                                                                                                                                 |
...
| `ValidationCheckResult`                                                                                                                                                                                                                                                                                                  | The outcome of `EditValidationState.check` — `allowed`, and `error` when it is not. See [cell editing](/v2/react/cell-editing/).                                                                                                                                                                                                                                                                                                                                                                           |
...
| `FeatureHostState`                                                                                                                                                                                                                                                                                                       | The registrations a feature's `setup` collected — filter types, queued patches, menu factories, side-panel entries — as the table reads them. See [feature composition](/v2/react/features/).                                                                                                                                                                                                                                                                                                              |
...
[customization](/v2/react/customization/#desktop-table-assembly).
...
entry) are the shared locale-resolution algorithm (see [i18n & RTL](/v2/react/i18n-rtl/)).
...
* `@adapttable/i18n` — `getLabels(locale)`, `getDirection(locale)`,
  ...
  … including `zhTW`) — see [i18n & RTL](/v2/react/i18n-rtl/).
