/**
 * "Omitting a prop renders nothing" — proved for Ant Design's toolbar extras.
 *
...
describe("Ant Design toolbar extras are opt-in", () => {
  it("draws no undo/redo without BOTH handlers", () => {
    const only = renderAntd(
    ...
    const neither = renderAntd(<UndoRedoButtons {...base} />);
  ...
  it("draws no export button without a handler", () => {
    const { container } = renderAntd(<ExportCsvButton {...base} />);
