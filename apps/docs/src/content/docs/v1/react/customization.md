---
title: "Customize AdaptTable — slots, classNames, headless prop-getters (v1)"
description: "Customize AdaptTable from props to fully headless: slots,
  classNames, data-* hooks, custom toolbars and TanStack-style prop-getters — no
  ejecting from the engine."
head:
  - tag: meta
    attrs:
      name: "robots"
  ...
  - tag: script
    ...
    content: '{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"AdaptTable","item":"https://adapttable.orwamahmoud.com/"},{"@type":"ListItem","position":2,"name":"Customize
      AdaptTable — slots, classNames, headless
      prop-getters","item":"https://adapttable.orwamahmoud.com/v1/react/customization/"}]}'
  - tag: meta
    attrs:
      property: og:image
...
Restyle without replacing. Mantine, Chakra, Radix, and Base UI expose five wrapper hooks
...
`data-adapttable-part="search-field"`; `rowsPerPageSelect` → `rows-per-page`,
`retryButton` → `retry`, and `pageButton` → `page-prev` / `page-next` /
`page-number`) — plus `data-*` state attributes, so
...
Opt-in via `onCellEdit` — see [Inline cell editing](/v1/react/cell-editing/). When
...
Opt-in via `groupBy` — see [Row grouping](/v1/react/row-grouping/). When grouping
...
`small`, Chakra `md` / `sm`, Radix and Base UI `"2"` / `"1"` — and MUI,
Chakra, antd, Radix, and Base UI offer an explicit `size` prop that
...
<DataTable
  ...
  stickyTop={56} // offset under your app header (Mantine also offsets its sticky toolbar)
...
* **Radix / Base UI** — `accentColor` colors primary controls (buttons,
  badges, active page); `size` (`"1" | "2" | "3"`).
...
See the [Columns guide](/v1/react/columns/) and the
[ColumnDef table](/v1/react/api/#columndef) for the full column surface
(`sortValue`, `i18n`, `group`, `hideOnMobile`, …).

To draw the markup yourself from the engine, see [headless](/v1/react/headless/). The CSV button and its helpers are on [CSV export](/v1/react/csv-export/).
