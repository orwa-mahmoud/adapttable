---
title: "React table URL state — shareable filters, sort, and page (v1)"
...
head:
  ...
  - tag: script
    ...
    content: '{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"AdaptTable","item":"https://adapttable.orwamahmoud.com/"},{"@type":"ListItem","position":2,"name":"React
      table URL state — shareable filters, sort, and
      page","item":"https://adapttable.orwamahmoud.com/v1/react/url-state/"}]}'
  - tag: meta
    attrs:
      property: og:image
      content: https://adapttable.orwamahmoud.com/og/url-state.png
  - tag: meta
    attrs:
      name: twitter:image
      content: https://adapttable.orwamahmoud.com/og/url-state.png
slug: v1/react/url-state
---

▶ **See it working:** [the live demo](https://adapttable.orwamahmoud.com/react/demo/) — filter, sort and page it, then copy the URL: every bit of that state is in the address bar (the kit switcher too).
...
| `groupBy`                   | `groupBy=department`        | Row-grouping column key (`groupBy`).                                                        |
...
| `colPin`                    | `colPin=name:start`         | Pinned columns and their side (`start` / `end`).                                            |
