---
title: "React table tree data — hierarchical rows (v2)"
description: Render hierarchical rows in a React data table — pass getChildren
  for expandable tree data with keyboard access and announcements.
head:
  - tag: meta
    attrs:
      name: "robots"
  ...
  - tag: script
    ...
    content: '{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"AdaptTable","item":"https://adapttable.orwamahmoud.com/"},{"@type":"ListItem","position":2,"name":"React
      table tree data — hierarchical
      rows","item":"https://adapttable.orwamahmoud.com/v2/react/tree-data/"}]}'
  - tag: meta
    attrs:
      property: og:image
      content: https://adapttable.orwamahmoud.com/og/tree-data.png
  - tag: meta
    attrs:
      name: twitter:image
      content: https://adapttable.orwamahmoud.com/og/tree-data.png
slug: v2/react/tree-data
---
...
real hierarchy, or a tree that reshuffles when someone sorts a column. When a
table sets both, grouping renders: the groups hold their rows without the tree's
chevrons or indents.
...
control that appears to do nothing. A rejection clears the flag and records the
node in `failedIds`; the next time the node is opened, the fetch runs again.
...
// useFilesQuery(params) wraps useInfiniteQuery; its params carry `expandedIds`.
const source = useQuerySource<Node>({
...
* `useTreeExpansion(options)` → `TreeExpansionState` — the open set, controlled
  or not, with `toggle`, `expand`, `expandAll`, `collapseAll`, `isExpanded`.
...
Every adapter stamps `data-adapttable-part` on each piece: `tree-cell` (the indented wrapper), `tree-toggle` (the chevron), and
