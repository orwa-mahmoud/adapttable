---
title: "material-table alternative — modern MUI, still MIT (v1)"
description: material-table is MUI v4-era and dormant. @adapttable/mui maps
  columns, remote data, actions and filters to a maintained Material UI table —
  plus what genuinely doesn't map.
head:
  - tag: meta
    attrs:
      name: "robots"
  ...
  - tag: script
    ...
    content: '{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"AdaptTable","item":"https://adapttable.orwamahmoud.com/"},{"@type":"ListItem","position":2,"name":"material-table
      alternative — modern MUI, still
      MIT","item":"https://adapttable.orwamahmoud.com/v1/react/migrate-from-material-table/"}]}'
  - tag: meta
    attrs:
      property: og:image
      content: https://adapttable.orwamahmoud.com/og/migrate-from-material-table.png
  - tag: meta
    attrs:
      name: twitter:image
      content: https://adapttable.orwamahmoud.com/og/migrate-from-material-table.png
slug: v1/react/migrate-from-material-table
---
...
* **Full row-edit CRUD suites** (`editable` as a material-table-style
  ...
  [Inline cell editing](/v1/react/cell-editing/)), not material-table's row
...
* **Drag-to-group aggregation** (`options.grouping`) — AdaptTable has
  single-level row grouping (`groupBy` + `groupAggregates`, frontend data
  only) and presentational header groups, but no drag-to-group UI or nested
  grouping.
* **Tree data** (`parentChildData`) — not supported.
* **PDF export button** — AdaptTable exports CSV only (the `exportCsv`
  toolbar button or the headless helpers).
...
| `localization`                                       | `labels` (+ [`@adapttable/i18n`](/v1/react/i18n-rtl/))      | Flat label object; presets for 17 locales incl. RTL.                 |
| `isLoading`                                          | `loading`                                             | —                                                                    |
...
| `sorting` / `defaultSort`          | `sortable`                                        | Default sort comes from URL state or a source builder's `defaults` (`useFrontendData({ defaults: { sortBy, sortDir } })`). |
...
| `cellStyle` / `headerStyle`        | `align`, `width`, `Cell` (+ table `rowClassName`) | Style through your own components/classes.                                                                                 |
...
* [Getting started](/v1/react/getting-started/) · [Data tiers](/v1/data-tiers/) ·
  [Filtering](/v1/react/filtering/) · [Selection & bulk actions](/v1/react/selection/).
* Same app also uses DataGrid or mui-datatables? See
  [Migrate from MUI X DataGrid](/v1/react/migrate-from-mui-x-datagrid/) and
  [Migrate from mui-datatables](/v1/react/migrate-from-mui-datatables/).
* [Comparison](/v1/react/comparison/) — where each library fits.
