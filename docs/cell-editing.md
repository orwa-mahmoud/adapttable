...
### Opening the row from your own pencil

A table that already has a pencil in its actions column does not want a second
control beside it saying the same thing. Mark that action `editsRow` and it
becomes the way in — the built-in **Edit** control stands down, and save and
...
only where there is a form to open: a table without `rowEditing` drops it rather
