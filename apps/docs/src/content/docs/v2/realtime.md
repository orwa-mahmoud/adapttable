---
title: "Realtime React data table — live row updates (v2)"
description: "Realtime React data table: patch rows as a websocket or SSE
  delivers them with useRowPatchStream, or apply them yourself with
  applyRowPatches. Keep the returned array so filters, sort, grouping and
  aggregates re-evaluate incrementally."
head:
  - tag: meta
    attrs:
      name: "robots"
  ...
  - tag: script
    ...
    content: '{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"AdaptTable","item":"https://orwa-mahmoud.github.io/adapttable/"},{"@type":"ListItem","position":2,"name":"Realtime
      React data table — live row
      updates","item":"https://orwa-mahmoud.github.io/adapttable/v2/realtime/"}]}'
  - tag: meta
    attrs:
      property: og:image
      content: https://orwa-mahmoud.github.io/adapttable/og/realtime.png
  - tag: meta
    attrs:
      name: twitter:image
      content: https://orwa-mahmoud.github.io/adapttable/og/realtime.png
...
**Related:** [Inline cell editing](/adapttable/v2/cell-editing/) · [API](/adapttable/v2/api/) ·
...
and `?patch=200&incremental=1` keeps the log. `node scripts/bench.mjs --only patch` prints the burst times. A 2026-08-26 run on this machine was
...
(1000 ms by default) and at most `reconnect.maxAttempts` times (unlimited by
default). An
...
`onPatches` is called after each batch is applied, with the patches that
arrived.
...
An insert marks the whole row; a remove has no cells left to mark. A mark
lasts `durationMs` (1200 ms by default); `flashProps(rowId, columnKey)` returns
the `data-flash` attribute for a cell you render yourself.
...
this page. That lives under [cell editing](/adapttable/v2/cell-editing/#live-update-conflicts).
...
* [API reference](/adapttable/v2/api/) lists `applyRowPatches`, `applyRowPatchesWithLog`
