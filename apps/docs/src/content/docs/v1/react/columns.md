---
title: "React table columns — ColumnDef, sort, pin, custom cells (v1)"
...
head:
  ...
  - tag: script
    ...
    content: '{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"AdaptTable","item":"https://adapttable.orwamahmoud.com/"},{"@type":"ListItem","position":2,"name":"React
      table columns — ColumnDef, sort, pin, custom
      cells","item":"https://adapttable.orwamahmoud.com/v1/react/columns/"}]}'
  - tag: meta
    attrs:
      property: og:image
      content: https://adapttable.orwamahmoud.com/og/columns.png
  - tag: meta
    attrs:
      name: twitter:image
      content: https://adapttable.orwamahmoud.com/og/columns.png
slug: v1/react/columns
---

▶ **Try it live:** [open a Mantine starter in StackBlitz](https://stackblitz.com/github/orwa-mahmoud/adapttable/tree/9f95391d5fd0f6f22585e8f0c9d2fc3c75da9cc0/starters/mantine?file=src%2FApp.tsx) — this page's feature is the starter's whole `columns` array in `src/App.tsx`; edit it in the browser, no install. [Other UI kits →](/v1/react/getting-started/#try-it-in-stackblitz)
...
* `sortable` opts a column into sorting; on frontend data the comparator reads `sortValue`, falling back to the column's accessor. See [sorting](/v1/react/sorting/).
...
* `group` labels a presentational header group: contiguous visible columns sharing a `group` render under one spanning header cell in a second header row. Groups are adjacency-based — reordering the columns apart splits the group.
* `filter` declares the column's filter widget (see [filtering](/v1/react/filtering/)); `editable` / `editor` / `editValue` opt the column into inline editing (see [cell editing](/v1/react/cell-editing/)).
...
| `sortValue`     | `(row) => SortableValue`         | the generated accessor value | Primitive extractor for the client-side sort. See [sorting](/v1/react/sorting/).        |
...
| `group`         | `string`                         | —                            | Presentational header group label shared by contiguous columns.                   |
| `filter`        | `ColumnFilter<TRow>`             | —                            | Declarative filter: a bare type or a definition. See [filtering](/v1/react/filtering/). |
| `editable`      | `boolean \| ((row) => boolean)`  | —                            | Opt into inline editing. See [cell editing](/v1/react/cell-editing/).                   |
| `editor`        | `CellEditor`                     | `"text"`                     | Editor widget when `editable` is set.                                             |
| `editValue`     | `(row) => string`                | `sortValue`, then key path   | Draft seed for the editor.                                                        |
...
Row clicks, row classes, density and the sticky header are on [row & display options](/v1/react/row-and-display-options/).
