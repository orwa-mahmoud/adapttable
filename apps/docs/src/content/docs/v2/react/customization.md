---
title: "Customize AdaptTable — classNames & slots (v2)"
description: Restyle parts with classNames, replace them with slots, tune the
  chrome with props, or theme through your kit provider. All opt-in.
head:
  - tag: meta
    attrs:
      name: "robots"
  ...
  - tag: script
    ...
    content: '{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"AdaptTable","item":"https://adapttable.orwamahmoud.com/"},{"@type":"ListItem","position":2,"name":"Customize
      AdaptTable — classNames &
      slots","item":"https://adapttable.orwamahmoud.com/v2/react/customization/"}]}'
  - tag: meta
    attrs:
      property: og:image
      content: https://adapttable.orwamahmoud.com/og/customization.png
  - tag: meta
    attrs:
      name: twitter:image
      content: https://adapttable.orwamahmoud.com/og/customization.png
slug: v2/react/customization
---

▶ **See it working:** [the unstyled adapter in the live demo](https://adapttable.orwamahmoud.com/react/demo/?kit=tailwind) — same engine, your own classes.
...
[row styling and heights](/v2/react/row-styling/).

## `classNames` — per-part styling

Restyle without replacing. The styled kits (Mantine, MUI, Chakra, Ant Design,
Radix, Base UI) expose five wrapper hooks (`root`, `toolbar`, `table`, `card`,
`footer`); the **unstyled** adapter
...
| `undoButton` / `redoButton` | The Undo and Redo buttons (`undoRedoButtons`).                          |
| `addRow`                    | The Add-row button (`onAddRow`).                                        |
| `printButton`               | The Print button (`printButton` with `onPrint`).                        |
| `densityToggle`             | The density toggle (`densityChooser`).                                  |
| `fullscreenToggle`          | The fullscreen toggle (`fullscreen`).                                   |
...
| `filtersForm`                                                                                                           | Wrapper around the auto form and the AND/OR builder.                                                               |
| `filterTree` / `filterTreeGroup` / `filterTreeCondition`                                                                | The AND/OR builder root, one nested group, and one condition row.                                                  |
| `filterTreeActions` / `filterTreeRemove` / `filterTreeSummary`                                                          | The add-condition/add-group row, a remove control, and the Advanced disclosure label.                              |
...
| `columnMenuSearch`                                          | The search box inside the column menu.                                  |
| `columnMenuBulk` / `columnMenuBulkButton`                   | The bulk show/hide/unpin row and one of its buttons.                    |
| `columnMenuMore` / `columnMenuSubmenu` / `columnMenuAction` | The per-column submenu trigger, the open submenu, and one action in it. |
...
| `viewsMenu`                                  | The menu container (trigger + anchored panel).                |
| `viewsRow` / `viewsSaveRow` / `viewsDivider` | One view's row, the save row, and the separator between them. |
...
| `selectionHeader`                                       | The leading select-all header cell.                                    |
| `columnSelect`                                          | The header checkbox that selects a column (`columnSelectionCheckbox`). |
| `bulkError`                                             | The failure line in the bulk bar after a rejected bulk action.         |
...
| `groupRow` / `groupCell`               | A row-grouping header `<tr>` and its spanning cell.                                          |
| `headerGroupRow` / `headerGroupCell`   | The column-group header `<tr>` above the column headers and one spanning `<th>`.             |
| `columnGroupToggle`                    | The collapse/expand control in a collapsible column-group header.                            |
| `headerActions`                        | Host-provided controls after a header caption.                                               |
...
| `actionsHeader`                        | The trailing row-actions header cell.                                                        |
| `cellMatch` / `cellMatchCurrent`       | A cell the find bar matched, and the match the find walk is on.                              |
| `fullWidthRow` / `fullWidthCell`       | A host-injected full-width `<tr>` and its spanning cell.                                     |
| `separatorRow` / `separatorCell`       | A host-injected separator `<tr>` and its spanning cell.                                      |
| `scrollBox`                            | The sideways/bounded scroll wrapper around the desktop table.                                |
| `virtualSpacer`                        | A virtualization padding spacer (desktop row or mobile card list).                           |
...
Opt-in via `onCellEdit` — see [Inline cell editing](/v2/react/cell-editing/). When
...
| `edit-cell-error`      | A validator's message under an open editor (a live region).         |
| `edit-cell-save-error` | A failed save's message under the cell (a live region).             |
| `edit-cell-rollback`   | The undo control beside it (`onEditRollback`).                      |

### Row grouping

Opt-in via `groupBy` — see [Row grouping](/v2/react/row-grouping/). When grouping
...
| `group-more`        | The row offering more groups, or more rows inside one.                |

### Tree data & row reordering

| Part                                                    | Element                                                                                  |
| ------------------------------------------------------- | ---------------------------------------------------------------------------------------- |
| `treeCell` / `treeToggle` / `treeSpacer`                | A tree row's indented wrapper, its fold chevron, and a leaf's chevron-width placeholder. |
| `reorderHeader` / `reorderCell` / `rowReorderHandle`    | The leading reorder header cell, body cell, and desktop drag handle.                     |
| `rowReorderButtons` / `rowReorderUp` / `rowReorderDown` | The mobile up/down control group and its two buttons.                                    |

### Palette, context menu, side panel & status bar

| Part                                                               | Element                                                                         |
| ------------------------------------------------------------------ | ------------------------------------------------------------------------------- |
| `commandPalette` / `commandInput` / `commandItem` / `commandEmpty` | The palette surface, its search box, one command, and the nothing-matched line. |
| `contextMenu` / `contextMenuItem` / `contextMenuSeparator`         | The right-click menu, one entry, and the divider between groups.                |
| `sidePanel` / `sidePanelTab` / `sidePanelClose`                    | The docked panel, one tab in its strip, and its close control.                  |
| `statusBar` / `statusItem`                                         | The status bar under the table and one figure in it.                            |
...
| `cardActions`                            | The trailing actions strip inside a card.          |
...
| `pagePrev` / `pageNext`                                   | The pager's previous- and next-page arrow buttons.                                            |
| `pageNumber` / `pageEllipsis`                             | One numbered page button (the current carries `aria-current="page"`) and the "…" elision gap. |
| `tableFooter`                                             | The free slot under the table, above the pager.                                               |
...
| `loadingTable` / `loadingHeaderRow` / `loadingHeaderCell` | The skeleton `<table>`, its header `<tr>`, and one header `<th>`.                             |
| `loadingRow` / `loadingCell` / `loadingLine`              | One skeleton body `<tr>`, one `<td>`, and the shimmering line inside it.                      |
| `loadingCards` / `loadingCard`                            | The skeleton mobile card list and one skeleton card.                                          |
...
it also takes a function — because an error state is *about* something:
...
`toolbar` fills the middle of the toolbar — between the search
...
`table.editHistory` stay the always-on path; this is the
...
no button, and a handler on its own is the palette's
...
in all eight kits and looks like the rest of your design system rather than
like ours.
...
`small`, Chakra `md` / `sm`, Radix and Base UI `"2"` / `"1"` — and MUI, Chakra,
antd, Radix and Base UI offer an explicit `size` prop that
...
* `scope: "all"` — the full filtered+sorted set when the source exposes it
  (frontend does); a server-backed source answers it through `request` or
  `fetchAll`, below.
...
* `scope: "range"` — the highlighted cell rectangle from
  [cell navigation](/v2/react/cell-navigation/). A rectangle names its own columns,
...
the same shape a [server tier](/v2/data-tiers/) receives, so an endpoint that
...
* **Radix / Base UI** — `accentColor` for primary controls; `size`
  (`"1" | "2" | "3"`) overrides the density mapping.
...
See the [Columns guide](/v2/react/columns/) and the
[ColumnDef table](/v2/react/api/#columndef) for the full column surface
