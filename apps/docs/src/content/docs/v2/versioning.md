---
title: "AdaptTable versioning & stability policy (v2)"
description: "Semantic versioning in practice: the committed-stable public API
  surface, how deprecations are handled, per-package releases."
head:
  - tag: meta
    attrs:
      name: "robots"
...
and `@adapttable/server` depend on `@adapttable/core` through a caret range
written at publish time (`^` the version they were built against;
`@adapttable/shadcn` depends on `@adapttable/unstyled` the same way), so you
do not need matching version numbers across kits — install the adapter you use
and npm resolves a compatible core. `@adapttable/cli` versions on its own cadence; its programmatic API
...
| `@adapttable/chakra`                          | `@chakra-ui/react` 3.13+ (+ `@emotion/react` 11) |
...
[CONTRIBUTING.md](https://github.com/orwa-mahmoud/adapttable/blob/55da84f73ec078c0a075b812710f1619ec300875/CONTRIBUTING.md) for the contributor flow.
