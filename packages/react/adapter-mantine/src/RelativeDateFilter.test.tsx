/**
 * The relative-date filter widget in Mantine's own controls.
 *
...
function RangeHarness({
  ...
  // Mantine's AutoFilterForm takes a whole TableSource, where the other kits
  // take the narrow filter bag, so the real frontend source is built and its
  // filter half overridden.
  const frontend = useFrontendData<{ id: string }>({ data: [], columns: [] });
  return (
    <AutoFilterForm
      defs={SHIPPED}
      ...
      source={{
        ...
        extra,
        setExtra: () => undefined,
        setExtras: (updates: ExtraFilters) => {
          onPatch(updates);
...
describe("Mantine relative date filter", () => {
