---
title: "ag-Grid alternative for CRUD — 300 kB lighter, MIT (v1)"
description: Shipping CRUD tables, not analytics grids? AdaptTable is ~300 kB
  lighter, renders your kit's real components, and includes URL state and filter
  UI under MIT — plus when to stay on AG Grid.
head:
  - tag: meta
    attrs:
      name: "robots"
  ...
  - tag: script
    ...
    content: '{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"AdaptTable","item":"https://adapttable.orwamahmoud.com/"},{"@type":"ListItem","position":2,"name":"ag-Grid
      alternative for CRUD — 300 kB lighter,
      MIT","item":"https://adapttable.orwamahmoud.com/v1/react/migrate-from-ag-grid/"}]}'
  - tag: meta
    attrs:
      property: og:image
      content: https://adapttable.orwamahmoud.com/og/migrate-from-ag-grid.png
  - tag: meta
    attrs:
      name: twitter:image
      content: https://adapttable.orwamahmoud.com/og/migrate-from-ag-grid.png
slug: v1/react/migrate-from-ag-grid
---
...
AdaptTable renders *your* UI kit's real components).

## When to stay on AG Grid

* Multi-level row grouping / pivoting / aggregation dashboards (AdaptTable
  covers single-level `groupBy` with per-group aggregates)
...
| `rowModelType: "infinite"`                         | `source={useBackendData(…)}`                          | Wraps your `useInfiniteQuery` hook. Paged server data: `onQueryChange` — one consolidated query; superseded fetches abort via `signal`. |
...
* **Don't migrate the analytics grids.** Multi-level grouping, pivoting,
  ...
  grouping with per-group aggregates is available via `groupBy` /
  `groupAggregates` (frontend data only; see [Row grouping](/v1/react/row-grouping/)).
* **Cell editing is opt-in, not spreadsheet-grade.** Map AG Grid `editable`
  columns to `ColumnDef.editable` + table `onCellEdit` (see
  [Inline cell editing](/v1/react/cell-editing/)). Full Excel-style fill-handle /
...
* **Row virtualization is opt-in** (`virtualize`) rather than always-on —
  ...
  [virtualization docs](/v1/react/virtualization/).

## Where next

* [Getting started](/v1/react/getting-started/) · [Data tiers](/v1/data-tiers/) ·
  [Filtering](/v1/react/filtering/) · [Column management](/v1/react/column-management/).
* [Comparison](/v1/react/comparison/) — the honest feature table, including where
  AG Grid wins.
