---
title: "React table virtualization — 50k rows, 24 nodes (v2)"
...
head:
  ...
  - tag: script
    ...
    content: '{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"AdaptTable","item":"https://adapttable.orwamahmoud.com/"},{"@type":"ListItem","position":2,"name":"React
      table virtualization — 50k rows, 24
      nodes","item":"https://adapttable.orwamahmoud.com/v2/react/virtualization/"}]}'
  - tag: meta
    attrs:
      property: og:image
      content: https://adapttable.orwamahmoud.com/og/virtualization.png
  - tag: meta
    attrs:
      name: twitter:image
      content: https://adapttable.orwamahmoud.com/og/virtualization.png
slug: v2/react/virtualization
---
...
* `rowHeight` overrides the estimate when set — a function is per row, so
  a variable-height table still windows. See [row styling and heights](/v2/react/row-styling/).
...
| `virtualizeColumns`   | `boolean` | `false` | Window the rendered columns on very wide tables (needs `maxHeight` or pinned columns). |
...
Reproduce both with
[`scripts/bench.mjs`](https://github.com/orwa-mahmoud/adapttable/blob/55da84f73ec078c0a075b812710f1619ec300875/scripts/bench.mjs)
...
* The same switch is a feature: `features={[virtualize()]}` from
  `@adapttable/<kit>/virtualize`. Pass an object to set the rest —
  `virtualizeColumns`, `estimateRowSize`, `estimateCardSize`,
  `virtualOverscan` and `virtualScrollMargin`. See
  [feature composition](/v2/react/features/).
* `virtualize` combines with `renderRowDetail`: on desktop each row is
  measured together with its open detail panel, and a panel that grows
  corrects the item's height. A mobile card holds its detail, so the card is
  the measured item.
* A paged table with `virtualize` renders unvirtualized, and a dev-mode
  warning says so.
