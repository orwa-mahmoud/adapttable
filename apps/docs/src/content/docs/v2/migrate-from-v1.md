---
title: "Migrate from AdaptTable v1 to v2 — every rename (v2)"
description: Migrate AdaptTable v1 to v2 — every rename and behavior change in
  one checklist, applied the same way across all eight adapters.
head:
  - tag: meta
    attrs:
      name: "robots"
  ...
  - tag: script
    ...
    content: '{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"AdaptTable","item":"https://orwa-mahmoud.github.io/adapttable/"},{"@type":"ListItem","position":2,"name":"Migrate
      from AdaptTable v1 to v2 — every
      rename","item":"https://orwa-mahmoud.github.io/adapttable/v2/migrate-from-v1/"}]}'
  - tag: meta
    attrs:
      property: og:image
      content: https://orwa-mahmoud.github.io/adapttable/og/migrate-from-v1.png
  - tag: meta
    attrs:
      name: twitter:image
      content: https://orwa-mahmoud.github.io/adapttable/og/migrate-from-v1.png
slug: v2/migrate-from-v1
---
...
adapters, and a batch of silent traps became loud. No v1 name ships as a
deprecated alias — v1 names were removed, not aliased, so the compiler walks
you through the rename table below.
...
* **The adapter-builder tier moved to `@adapttable/core/adapter`.** The
  ...
  app-facing API. *Why: autocomplete and docs at the main entry now show
  only what app code is meant to use.*
...
* **Explicit `mode` prop.** `mode="server"` requires `onQueryChange` at
  ...
  keeps v1 inference exactly. *Why: the frontend tier had no way to
  observe query changes without surrendering the data path.*

* **`onGroupByChange` and `onClearFilters` are observers.** The table
  always performs the change itself, then notifies. *Why: providing a
  logging handler used to silently replace the built-in mutation.* Take
...
* **`baseParams` never beat live state, and filters are namespaced.**
  ...
  spread at the top level. *Why: a static param could silently override an
  active filter, and a filter named `sortBy` clobbered the sort.*
...
* **Packaging**: truthful peer floors (Chakra `^3.13`, MUI `^6.1.2`,
