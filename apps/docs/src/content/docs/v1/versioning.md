---
title: "AdaptTable versioning & stability policy (v1)"
description: AdaptTable's versioning and stability policy — semantic versioning,
  the committed-stable public API surface, deprecation policy and independent
...
depend on `@adapttable/core` through a caret range of the version current at
publish time (`@adapttable/shadcn` depends on `@adapttable/unstyled` the same
way), so
...
* **`@adapttable/core`** — the `TableSource` contract; the `useFrontendData` /
  ...
  and virtualization hooks; the filter primitives (count-filter helpers such as `countFilterExtra`,
  chips, range widget); and the i18n-agnostic labels contract.
...
[CONTRIBUTING.md](https://github.com/orwa-mahmoud/adapttable/blob/9f95391d5fd0f6f22585e8f0c9d2fc3c75da9cc0/CONTRIBUTING.md) for the contributor flow.
