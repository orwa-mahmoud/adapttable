---
title: "mui-datatables alternative — maintained, MUI v6+, React 19 (v1)"
description: mui-datatables is unmaintained since Jan 2023 and stuck on MUI v5.
  @adapttable/mui maps columns and onTableChange to one query callback — MUI v6+
  and React 19 ready. Migration guide inside.
head:
  - tag: meta
    attrs:
      name: "robots"
  ...
  - tag: script
    ...
    content: '{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"AdaptTable","item":"https://adapttable.orwamahmoud.com/"},{"@type":"ListItem","position":2,"name":"mui-datatables
      alternative — maintained, MUI v6+, React
      19","item":"https://adapttable.orwamahmoud.com/v1/react/migrate-from-mui-datatables/"}]}'
  - tag: meta
    attrs:
      property: og:image
  ...
  - tag: meta
    attrs:
      name: twitter:image
      content: https://adapttable.orwamahmoud.com/og/migrate-from-mui-datatables.png
slug: v1/react/migrate-from-mui-datatables
---
...
overview](/v1/react/getting-started/).
...
| `options.setCellProps` / `setCellHeaderProps`     | `align`, `width` (+ table `rowClassName`) | Common cases are first-class props.                                    |
...
* **`textLabels` → `labels`**, and locale presets (incl. RTL languages) come
  from [`@adapttable/i18n`](/v1/react/i18n-rtl/).
...
* Also migrating the grid elsewhere in your app? See
  [Migrate from MUI X DataGrid](/v1/react/migrate-from-mui-x-datagrid/).
