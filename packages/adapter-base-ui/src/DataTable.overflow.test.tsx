...
function renderTable(stickyTop?: number): HTMLElement {
  ...
  // The wrapper Box is the `overflowRef` parent of the Base UI Table.Root.
  return container.querySelector("table")!.closest(".adapttable-table-root")!
    .parentElement!;
}
...
describe("column-pin sticky fix (Base UI ScrollArea workaround)", () => {
  it("restores the inner table's overflow and pushes the fixed-column min-width onto it", () => {
    ...
    const wrapper = container
      .querySelector("table")!
      .closest(".adapttable-table-root")!.parentElement!;
    // The wrapper scopes the override and ships the rule that beats Base UI's
    // `surface` overflow:hidden and gives the inner <table> its min-width.
    expect(wrapper.classList.contains("adapttable-base-ui-scroll")).toBe(true);
    const rule = wrapper.querySelector("style")!.textContent ?? "";
    expect(rule).toContain("table{overflow:visible");
    ...
    // reads, so the table (not Base UI's ScrollArea viewport) is what overflows —
    // which is what lets the pinned/edge sticky cells stick.
    expect(wrapper.style.getPropertyValue("--adapttable-min-width")).toMatch(
      /^\d+px$/
    );
    // Base UI's Table.Root must NOT carry an inline min-width that would keep the
    // viewport (and not the table) as the thing that overflows.
    const root = wrapper.querySelector<HTMLElement>(".adapttable-table-root")!;
