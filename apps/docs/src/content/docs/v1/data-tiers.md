---
title: "Client & server React table data — one TableSource API (v1)"
...
* **Server**: the table owns page, page size, debounced search, sort, and
  ...
  job is to fetch and hand back `data` + `total`. The server tier always
  paginates in paged mode.
...
* The `data` / `onQueryChange` tiers take no `error`, `paginationMode`,
  `defaults`, `filterFn`, `getSearchText` or `getSortValue` props — those are
  options on the source builders; build a `source` to use them.
* `useBackendData` accepts `selectPage` (when your page shape isn't
  ...
  [`examples/mui-backend.tsx`](https://github.com/orwa-mahmoud/adapttable/blob/9f95391d5fd0f6f22585e8f0c9d2fc3c75da9cc0/examples/mui-backend.tsx) for a complete
  runnable version.
* On the server tier, `source.refetch()` re-emits the current query; stale
  responses are handled for you via the abort signal. Out-of-range pages are
