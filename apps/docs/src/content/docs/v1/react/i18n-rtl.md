---
title: "React table i18n & RTL — Arabic, Hebrew, 17 locales (v1)"
description: "React table with first-class RTL/Arabic: locale presets,
  per-locale column paths, logical pinning and mirrored layout — not just
  translated strings."
head:
  - tag: meta
    attrs:
  ...
  - tag: script
    ...
    content: '{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"AdaptTable","item":"https://adapttable.orwamahmoud.com/"},{"@type":"ListItem","position":2,"name":"React
      table i18n & RTL — Arabic, Hebrew, 17
      locales","item":"https://adapttable.orwamahmoud.com/v1/react/i18n-rtl/"}]}'
  - tag: meta
    attrs:
      property: og:image
      content: https://adapttable.orwamahmoud.com/og/i18n-rtl.png
  - tag: meta
    attrs:
      name: twitter:image
      content: https://adapttable.orwamahmoud.com/og/i18n-rtl.png
slug: v1/react/i18n-rtl
---

▶ **See it working:** [flip the whole table to Arabic RTL in the live demo](https://adapttable.orwamahmoud.com/react/demo/rtl/) — a real table you can interact with, not a recording.
...
| `zhTW` (`"zh-TW"`) | Chinese (Traditional) | ltr       |
...
* Column pinning is logical: pins use `insetInlineStart`/`insetInlineEnd`,
  so a `start` pin sticks to the inline-start edge in RTL too.
* Column resizing is direction-aware: the handle sits on the column's
  inline-end edge, and dragging outward (or pressing the arrow key that points
  toward the inline end — ArrowRight in LTR, ArrowLeft in RTL) widens the column in both LTR and RTL.
