---
title: "React table filtering — chips & URL-synced (v2)"
description: Declare a filter once and AdaptTable derives the kit-native widget,
  the URL param, the removable chip and the row predicate.
head:
  - tag: meta
    attrs:
      name: "robots"
  ...
  - tag: script
    ...
    content: '{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"AdaptTable","item":"https://orwa-mahmoud.github.io/adapttable/"},{"@type":"ListItem","position":2,"name":"React
      table filtering — chips &
      URL-synced","item":"https://orwa-mahmoud.github.io/adapttable/v2/filtering/"}]}'
  - tag: meta
    attrs:
      property: og:image
      content: https://orwa-mahmoud.github.io/adapttable/og/filtering.png
  - tag: meta
    attrs:
      name: twitter:image
      content: https://orwa-mahmoud.github.io/adapttable/og/filtering.png
...
▶ **Try it live:** [open a Mantine starter in StackBlitz](https://stackblitz.com/github/orwa-mahmoud/adapttable/tree/55da84f73ec078c0a075b812710f1619ec300875/starters/mantine?file=src%2FApp.tsx) — this page's feature is already wired in `src/App.tsx` (declarative `filter` widgets on four columns); edit it in the browser, no install. [Other UI kits →](/adapttable/v2/getting-started/#try-it-in-stackblitz)
...
the [advanced filter tree](/adapttable/v2/filter-tree/).
...
* Widgets are operator-first. Text offers equals / not equals / contains /
  ...
  on / on-or-after / on-or-before / between / relative window / empty. The operator token is
  ...
  A link without `Op` still resolves: text defaults to contains, and a
  Min/Max pair infers at-least / at-most / between.
...
* Active filters render as removable chips with a clear-all that resets every
  filter (and the page) and drops the AND/OR tree while search and sort
  survive; `onClearFilters` is a notification fired after that clear (take
  full control through `source.clearExtras`).
...
| `column`      | `string`                                                    | `key`                 | Column the header filter row places the widget under, when it differs from `key`.                |
...
| `filtersMode`               | `"popover" \| "drawer" \| "header"` | `"popover"`    | One container. Popover: anchored card, no backdrop. Drawer: panel + backdrop. Header: compact per-column row; hides the Filters button unless `source.setFilterTree` is set. |
| `onClearFilters`            | `() => void`                        | —              | Notification fired after the drawer, chip strip or no-results action clears the filters. The table performs the clear itself.                                                |
...
| `filterDefs`                | `FilterDef[]`                       | from `filters` | Resolved definitions that label AND/OR tree chips; set by the shell, passed by hosts that call `useTableChrome` directly.                                                    |
...
* **Count filters** (the numeric operator + value pair, committed stable in
  [Versioning & stability](/adapttable/v2/versioning/)): `COUNT_OPERATORS` is the operator
...
* **Operators**: `TEXT_OPS` / `NUMBER_OPS` / `DATE_OPS` are the stable URL
  ...
  Clear all drops `ft`. See [filter-tree](/adapttable/v2/filter-tree/).
* **Facet counts**: `computeFilterFacets` / `rowsExcludingFilter` /
  `FacetMap` / `FacetCounts` count what selecting a value *would* keep —
...
* **Type registry**: `FilterTypeSpec` is one type — widget kind, operators,
  ...
  `extend` are deprecated; prefer `TableFeatureHost.registerFilterType`
...
* **Range widgets**: `useRangeFilterWidget` is the kit-agnostic logic behind
  ...
  `RANGE_OPS` is the four-operator set (`eq` / `gte` / `lte` /
...
* **Search**: `defaultSearchText` is the default searchable-text projector —
  ...
  The search box itself is on by default (`searchable={false}` hides it);
  `searchPlaceholder` sets its placeholder, and `searchDebounceMs` (300 by
  default) is the delay before a typed term commits to the source and the
  `q` URL param.
