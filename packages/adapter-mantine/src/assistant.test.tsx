 ...
 * from Mantine, and that the structure the chrome owns arrives intact.
 */
...
describe("TableAssistant", () => {
  ...
  it("draws its controls with Mantine's own components", () => {
    mount(
      <TableAssistant
        assistant={view()}
        labels={defaultLabels}
    ...
    // look, which is the same defect as drawing the control in core. Send is
    // an icon control, so it is Mantine's ActionIcon rather than its Button.
    expect(part("assistant-send")!.className).toContain(
