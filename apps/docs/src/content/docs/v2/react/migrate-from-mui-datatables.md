---
title: "Migrate from mui-datatables — maintained, v6+ (v2)"
description: mui-datatables has had no releases since January 2023 and stops at
  MUI v5. @adapttable/mui is MUI v6+ and React 19 ready.
head:
  - tag: meta
    attrs:
      name: "robots"
  ...
  - tag: script
    ...
    content: '{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"AdaptTable","item":"https://adapttable.orwamahmoud.com/"},{"@type":"ListItem","position":2,"name":"Migrate
      from mui-datatables — maintained,
      v6+","item":"https://adapttable.orwamahmoud.com/v2/react/migrate-from-mui-datatables/"}]}'
  - tag: meta
    attrs:
      property: og:image
      content: https://adapttable.orwamahmoud.com/og/migrate-from-mui-datatables.png
  - tag: meta
    attrs:
      name: twitter:image
      content: https://adapttable.orwamahmoud.com/og/migrate-from-mui-datatables.png
slug: v2/react/migrate-from-mui-datatables
---

▶ **See it before you install:** [the live demo running on real Material UI](https://adapttable.orwamahmoud.com/react/demo/?kit=mui) — nothing to set up.
...
**MUI v6 (6.1.2+) through v9** — so migrating unfreezes your MUI upgrade path while
...
overview](/v2/react/getting-started/).
...
| `options.download` / `print`                          | `exportCsv` prop / `rowsToCsv` + `downloadCsv`        | Built-in Export CSV button, or headless helpers on a custom `toolbar` button; print via `onPrint` (+ `printButton`) with `printTable` from `@adapttable/core/pdf`. |
...
| `options.setCellProps` / `setCellHeaderProps`     | `align`, `width`, `rowClassName` / `rowStyle` | Common cases are first-class column props; per-row classes and styles are table-level; the MUI adapter's `classNames` covers its wrapper parts. |
...
**After** — `@adapttable/mui` (typed, URL-synced, MUI v6–v9):
...
* **CSV export is built-in and headless.** Pass `exportCsv` for a toolbar
  button, or wire `rowsToCsv` / `downloadCsv` (from `@adapttable/core`) yourself via
  the `toolbar` slot. For print, wire `onPrint` — with `printButton` for a
  toolbar control — to `printTable` from `@adapttable/core/pdf`.
* **`textLabels` → `labels`**, and locale presets (incl. RTL languages) come
  from [`@adapttable/i18n`](/v2/react/i18n-rtl/).
...
* Also migrating the grid elsewhere in your app? See
  [Migrate from MUI X DataGrid](/v2/react/migrate-from-mui-x-datagrid/).
