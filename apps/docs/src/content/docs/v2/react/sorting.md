---
title: "React table sorting — multi-column, URL-synced (v2)"
...
head:
  ...
  - tag: script
    ...
    content: '{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"AdaptTable","item":"https://adapttable.orwamahmoud.com/"},{"@type":"ListItem","position":2,"name":"React
      ...
      URL-synced","item":"https://adapttable.orwamahmoud.com/v2/react/sorting/"}]}'
  - tag: meta
    attrs:
      property: og:image
  ...
  - tag: meta
    attrs:
      name: twitter:image
      content: https://adapttable.orwamahmoud.com/og/sorting.png
slug: v2/react/sorting
---

▶ **Try it live:** [open a Mantine starter in StackBlitz](https://stackblitz.com/github/orwa-mahmoud/adapttable/tree/55da84f73ec078c0a075b812710f1619ec300875/starters/mantine?file=src%2FApp.tsx) — this page's feature is already wired in `src/App.tsx` (`sortable` columns); edit it in the browser, no install. [Other UI kits →](/v2/react/getting-started/#try-it-in-stackblitz)
...
* Frontend tier (`data` / `useFrontendData`): rows are compared by `sortValue`, falling back to the column's accessor. Numbers compare numerically, booleans `false` before `true`, everything else by locale-aware string comparison; the sort is stable.
...
* Every sort change returns the table to page 1.
...
| `sortByOptions` | `SortByOption[]`                           | derived from the columns     | Options for the mobile sort-by select. Omitted, every `sortable` column with a string `header` (else `mobileLabel`) contributes one. |
...
* A custom caption (`renderHeader`) receives a `controller` with `sortDir`, the 1-based `sortIndex` and `toggleSort(event)` (pass `{ shiftKey: true }` to chain), so it stays wired to the same state.
* A settled sort is announced politely to screen readers — `labels.sortedBy({ column, ascending })`, and `labels.sortingCleared` when the last sort is removed.
