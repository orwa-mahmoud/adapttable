---
title: "React table keyboard navigation — ARIA grid (v2)"
description: "Arrow-key cell navigation for a React table: one tab stop, correct
  ARIA grid semantics, absolute row indices under virtualization, and spoken
  announcements."
head:
  - tag: meta
    attrs:
  ...
  - tag: script
    ...
    content: '{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"AdaptTable","item":"https://adapttable.orwamahmoud.com/"},{"@type":"ListItem","position":2,"name":"React
      table keyboard navigation — ARIA
      grid","item":"https://adapttable.orwamahmoud.com/v2/react/cell-navigation/"}]}'
  - tag: meta
    attrs:
      property: og:image
      content: https://adapttable.orwamahmoud.com/og/cell-navigation.png
  - tag: meta
    attrs:
      name: twitter:image
      content: https://adapttable.orwamahmoud.com/og/cell-navigation.png
slug: v2/react/cell-navigation
---
...
**Related:** [Accessible React data table](/v2/react/accessibility/) ·
[Cell editing](/v2/react/cell-editing/) · [Virtualization](/v2/react/virtualization/) ·
...
`@adapttable/core`. Each adapter mounts `FindBar` over `FindBarChrome` from
...
`statusBar` puts the same figures in a status bar under the table, beside the
counts of rows on screen and rows selected; the sums appear only with
`selectionStats` set.
...
| `Ctrl`/`Cmd`+`Z`                      | Undo, when `editHistory` is set                 |
| `Ctrl`/`Cmd`+`Shift`+`Z` / `Ctrl`+`Y` | Redo, when `editHistory` is set                 |
| `Ctrl`/`Cmd`+`F`                      | Opens the find bar, when `findInTable` is set   |
...
The cell's text comes from [`columnText`](/v2/react/columns/), so a column whose cell
...
Shift+Down twice then Shift+Up *shrink* the range instead of starting a new one
...
`table.gridFocus.range` holds the current rectangle — which is what `exportCsv`
with `scope: "range"` reads — and `useGridFocus` takes an `onRangeChange`
callback that fires whenever it changes.
