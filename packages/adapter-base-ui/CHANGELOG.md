...
- 85aaa30: Mobile card move controls are 44 px touch targets in every kit, with each kit's own button and glyph.
...
- fcabe6c: Every kit's `<DataTable>` types `supports`, `facetKeys` and `facets`, and Chakra UI, Radix and Base UI type `error`, matching Ant Design. A server-tier table can declare facet and aggregate support on any kit.
...
- cd45219: Move from v2's React-based core to a framework-neutral engine with an optional
...
- f8ba086: Keep the value the user chose in a select whose option list depends on that value. Choosing a new
