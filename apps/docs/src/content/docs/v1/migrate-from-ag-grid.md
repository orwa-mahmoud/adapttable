...
head:
  ...
  - tag: script
    ...
    content: '{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"AdaptTable","item":"https://orwa-mahmoud.github.io/adapttable/"},{"@type":"ListItem","position":2,"name":"ag-Grid
      alternative for CRUD — 300 kB lighter,
      MIT","item":"https://orwa-mahmoud.github.io/adapttable/v1/migrate-from-ag-grid/"}]}'
  - tag: meta
    attrs:
      property: og:image
...
* **Don't migrate the analytics grids.** Multi-level grouping, pivoting,
  ...
  `groupAggregates` (frontend data only; see [Row grouping](/adapttable/v1/row-grouping/)).
* **Cell editing is opt-in, not spreadsheet-grade.** Map AG Grid `editable`
  columns to `ColumnDef.editable` + table `onCellEdit` (see
  [Inline cell editing](/adapttable/v1/cell-editing/)). Full Excel-style fill-handle /
...
* **Row virtualization is opt-in** (`virtualize`) rather than always-on —
  ...
  [virtualization docs](/adapttable/v1/virtualization/).

## Where next

* [Getting started](/adapttable/v1/getting-started/) · [Data tiers](/adapttable/v1/data-tiers/) ·
  [Filtering](/adapttable/v1/filtering/) · [Column management](/adapttable/v1/column-management/).
* [Comparison](/adapttable/v1/comparison/) — the honest feature table, including where
  AG Grid wins.
