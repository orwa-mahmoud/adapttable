 ...
 * from Radix Themes, and that the structure the chrome owns arrives intact.
 */
...
describe("TableAssistant", () => {
  ...
  it("opens the examples from the composer, in this kit's own menu", async () => {
    ...
    // Radix opens its menu on pointer-down, not on a synthesised click.
  ...
  it("draws its controls with Radix Themes's own components", () => {
    mount(
      <TableAssistant
        assistant={view()}
        labels={defaultLabels}
    ...
    expect(part("assistant-send")!.className).toContain("rt-IconButton");
  });
});
