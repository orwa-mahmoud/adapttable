---
title: "TanStack Table alternative — headless + ready UI kits (v1)"
description: Love TanStack's headless model but tired of rebuilding filters,
  toolbars, pagination and URL sync? AdaptTable keeps prop-getters and ships
  native adapters for eight React UI kits. Migration map inside.
head:
  - tag: meta
    attrs:
      name: "robots"
  ...
  - tag: script
    ...
    content: '{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"AdaptTable","item":"https://orwa-mahmoud.github.io/adapttable/"},{"@type":"ListItem","position":2,"name":"TanStack
      Table alternative — headless + ready UI
      kits","item":"https://orwa-mahmoud.github.io/adapttable/v1/migrate-from-tanstack-table/"}]}'
  - tag: meta
    attrs:
      property: og:image
      content: https://orwa-mahmoud.github.io/adapttable/og/migrate-from-tanstack-table.png
  - tag: meta
    attrs:
      name: twitter:image
      content: https://orwa-mahmoud.github.io/adapttable/og/migrate-from-tanstack-table.png
slug: v1/migrate-from-tanstack-table
---
...
* **A filter UI.** TanStack gives you `columnFilters` state and
  ...
  declaration. See [filtering](/adapttable/v1/filtering/).
...
* **URL-synced state.** TanStack has no URL API; you serialize
  ...
  [URL state](/adapttable/v1/url-state/).
* **Saved views** — no concept in TanStack. See [saved views](/adapttable/v1/saved-views/).
...
| `manualSorting` / `manualFiltering` / `manualPagination`              | `onQueryChange` (or `source` via `useBackendData`)                      | One consolidated server query. See [data tiers](/adapttable/v1/data-tiers/). |
...
* [Concepts](/adapttable/v1/concepts/) — the headless engine and `TableSource`.
* [Getting started](/adapttable/v1/getting-started/) · [Filtering](/adapttable/v1/filtering/) ·
  [Data tiers](/adapttable/v1/data-tiers/) · [API reference](/adapttable/v1/api/).
