---
title: "mantine-datatable alternative — same Mantine, more built-in (v1)"
description: Need more than mantine-datatable? @adapttable/mantine keeps real
  Mantine components and adds URL-synced state, filter chips, column management,
  saved views and virtualization. Prop-by-prop migration map.
head:
  - tag: meta
    attrs:
      name: "robots"
  ...
  - tag: script
    ...
    content: '{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"AdaptTable","item":"https://orwa-mahmoud.github.io/adapttable/"},{"@type":"ListItem","position":2,"name":"mantine-datatable
      alternative — same Mantine, more
      built-in","item":"https://orwa-mahmoud.github.io/adapttable/v1/migrate-from-mantine-datatable/"}]}'
  - tag: meta
    attrs:
      property: og:image
      content: https://orwa-mahmoud.github.io/adapttable/og/migrate-from-mantine-datatable.png
  - tag: meta
    attrs:
      name: twitter:image
      content: https://orwa-mahmoud.github.io/adapttable/og/migrate-from-mantine-datatable.png
slug: v1/migrate-from-mantine-datatable
---
...
* **A real filter UI.** Declare `filter` on a column and get a kit-native
  ...
  [filtering](/adapttable/v1/filtering/).
...
* **URL-synced, shareable, reload-safe state** — mantine-datatable keeps state
  in React only. See [URL state](/adapttable/v1/url-state/).
* **A ready Columns menu** (show/hide, reorder, pin) via `enableColumnMenu`, and
  ...
  [column management](/adapttable/v1/column-management/).
* **Saved views, an automatic mobile card layout, and opt-in
  virtualization** — none of which mantine-datatable ships. See
  [saved views](/adapttable/v1/saved-views/) and [virtualization](/adapttable/v1/virtualization/).
...
| `page` / `onPageChange` / `totalRecords`      | automatic (frontend) or `total` + `onQueryChange` (server) | Frontend pagination is `"auto"`: paged on desktop, infinite on mobile; the `onQueryChange` tier is paged. |
...
* **Stop sorting and slicing `records`.** On the frontend tier, hand the full
  ...
  table does it. For a server API, use the [server tier](/adapttable/v1/data-tiers/)
...
* [Getting started](/adapttable/v1/getting-started/) — install, providers, first table.
* [Filtering](/adapttable/v1/filtering/) · [Data tiers](/adapttable/v1/data-tiers/) ·
  [Column management](/adapttable/v1/column-management/) · [Saved views](/adapttable/v1/saved-views/).
