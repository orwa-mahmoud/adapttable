...
- **Feature composition** (`features={[rowReorder(fn)]}`) from `@adapttable/radix/row-reorder`-style subpaths. Enabling props still work until v3; no bundle saving yet. Host plugins share the same `setup(host)` surface.
