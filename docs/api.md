...
- `createTableViewStore(config: TableViewStateConfig): TableViewStore` — the
  store over a `UrlStateAdapter` for everything the table writes to the URL:
  `getSnapshot()`, `subscribe()` and the mutators. `TableViewState` is its
  snapshot, `ViewStateSource` / `ViewStateSubscription` how a binding feeds it.
- `createUrlSliceStore(spec: UrlSliceSpec): UrlSliceStore` — one debounced URL
  slice (`URL_SLICE_WRITE_DEBOUNCE_MS`). The slices the table ships:
  `columnLayoutSlice`, `densitySlice` (`TableDensity`), `groupCollapseSlice`,
  `rowPinningSlice`, `pivotSlice` and `formulaSlice`.
- `resolveUrlAdapter(adapter, enabled, memory)` — the adapter a table uses:
  the host's, the browser history, or a memory store when URL sync is off.
  `resetHistoryAdapter()` drops the shared history adapter, for tests.
...
- `FirstLoadLatch` / `createFirstLoadLatch` — holds the loading state until the
  first answer. `clampedPage(page, total, limit)` — the last page that exists.
...
- `createControllableStore(options: ControllableStoreOptions): ControllableStore`
  — a value that is the host's when controlled and the store's own otherwise,
  reporting every change. `ControllableControl` is the controlled half.
...
- `createEditHistoryStack(depth?): EditHistoryStack` — undo and redo over
  edit gestures, `DEFAULT_EDIT_HISTORY_DEPTH` deep. `EditHistorySnapshot`,
