---
title: "React table row grouping with subtotals (v2)"
description: Nested React table row grouping — groupBy takes one key or an
  ordered list, per-group aggregates sharing the summaryRow mapper,
  expand/collapse, frontend tier only.
head:
  - tag: meta
    attrs:
      name: "robots"
  ...
  - tag: script
    ...
    content: '{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"AdaptTable","item":"https://adapttable.orwamahmoud.com/"},{"@type":"ListItem","position":2,"name":"React
      table row grouping with
      subtotals","item":"https://adapttable.orwamahmoud.com/v2/react/row-grouping/"}]}'
  - tag: meta
    attrs:
      property: og:image
      content: https://adapttable.orwamahmoud.com/og/row-grouping.png
  - tag: meta
    attrs:
      name: twitter:image
      content: https://adapttable.orwamahmoud.com/og/row-grouping.png
slug: v2/react/row-grouping
---

▶ **Try it live:** [open a Mantine starter in StackBlitz](https://stackblitz.com/github/orwa-mahmoud/adapttable/tree/55da84f73ec078c0a075b812710f1619ec300875/starters/mantine?file=src%2FApp.tsx) — this page's feature is already wired in `src/App.tsx` (`groupBy="role"` + `groupAggregates`); edit it in the browser, no install. [Other UI kits →](/v2/react/getting-started/#try-it-in-stackblitz)

▶ **See it working:** [collapse groups and read per-group subtotals in the live demo](https://adapttable.orwamahmoud.com/react/demo/mantine/grouping/) — a real table you can click, not a recording.
...
```tsx
// usePeopleQuery(params) wraps useInfiniteQuery; its params carry `groupBy`
// and `aggregates`.
const people = useQuerySource<Person>({
  usePaginatedQuery: usePeopleQuery,
  defaults: { groupBy: "team" },
  supports: { grouping: true, aggregates: true },
  aggregates: [{ key: "budget", fn: "sum" }],
});
// `groups`: the `groups` field of the latest response.
const source = { ...people, groups };

<DataTable source={source} columns={columns} rowKey={rowKey} />;
```
...
* **One key or a list.** `groupBy="team"` groups one level;
  ...
  the URL.
* **Frontend tier, or a server that groups.** Grouping needs the full filtered
  row set in memory (`allFilteredRows`) or groups the server computed (`groups`
  on the source, see [Grouping on the server](#grouping-on-the-server)). Other
  server-paginated sources log a dev-mode warning and ignore grouping — see
...
* **Shared mapper.** `groupAggregates(rows)` uses the same
  ...
  `aggregate()` (below).
* **Expand / collapse.** Groups start expanded. Built-in collapse state is
  ephemeral (not URL-synced); `useGroupCollapseUrlState` puts it in the URL.
  `groupBy` itself serializes to the URL like sort and filters.
...
| --------------------------- | ------------------------------------------------------------------ | ------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| `groupBy`                   | `string \| readonly string[] \| null`                              | —       | Column key, or keys outermost first, to group by; its presence arms grouping (still requires a frontend data source or server groups).  |
| `onGroupByChange`           | `(groupBy: readonly string[]) => void`                             | —       | Notification after the table applies a grouping change; empty when grouping was cleared. Take full control through `source.setGroupBy`. |
...
| `groupFooters`              | `boolean`                                                          | `false` | Close every group with a footer row carrying its `groupAggregates`.                                                                     |
| `groupSort`                 | `"label" \| "label-desc" \| "count" \| "count-desc" \| comparator` | —       | Order groups within their parent; without it groups keep the source's order.                                                            |
...
| `groupRowPageSize`          | `number`                                                           | —       | Show at most this many rows inside each group, with a "more in this group" row.                                                         |
...
| `windowGroupedEntries`                         | Slice a flat grouped model to a virtual window (see [Virtualization](/v2/react/virtualization/)).    |
...
* Out of scope (by design): a drag-to-group panel and Excel-style aggregation
  pickers. Pivoting is a separate model — see [Pivot](/v2/react/pivot/).
