---
title: "React table data — client, server, one API (v2)"
description: One React table API for in-memory rows and server-paginated APIs.
  Swap client data for a fetch function without rewriting the UI — TableSource
  is the contract.
head:
  - tag: meta
    attrs:
...
interface TableQuery {
  page: number; // 1-based
  limit: number;
  search: string; // committed (debounced) term
...
| `expandedIds` | `tree`       | Ids of the open tree nodes, so the response can carry their children                                                                                                                                                           |
...
* **`tableQueryBaseKey`** covers what decides *which* rows: search, filters,
  sort, grouping, page size.
* **`tableQueryKey`** appends *where* in them the table is: page and cursor.
...
| `mode`          | `"frontend" \| "server"`                                                      | —       | Pins the tier explicitly; omitted, the tier is inferred. See [Explicit `mode`](#explicit-mode--when-inference-isnt-what-you-meant). |
...
* `useQuerySource` accepts `selectPage` (a `PageSelector` — project your
  own page shape to `{ rows, total?, facets? }` when it isn't `PaginatedResponse`),
  `baseParams` (static params merged into every call, e.g. a parent scope
  id), and `sanitizeParams`, plus `supports`, `facetKeys`, `aggregates`,
  `expandedIds` and `nextCursor` (a `(page) => string | null | undefined`
  reader) for the query-contract capabilities above. Its query argument is typed structurally as
  `InfiniteQueryLike`, so TanStack Query stays a type-only peer. See
  [`examples/mui-query-source.tsx`](https://github.com/orwa-mahmoud/adapttable/blob/55da84f73ec078c0a075b812710f1619ec300875/examples/mui-query-source.tsx) for a complete
  runnable version.
