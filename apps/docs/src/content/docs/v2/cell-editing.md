...
head:
  ...
  - tag: script
    ...
    content: '{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"AdaptTable","item":"https://orwa-mahmoud.github.io/adapttable/"},{"@type":"ListItem","position":2,"name":"React
      ...
      onCellEdit","item":"https://orwa-mahmoud.github.io/adapttable/v2/cell-editing/"}]}'
  - tag: meta
    attrs:
      property: og:image
  ...
  - tag: meta
    attrs:
      name: twitter:image
      content: https://orwa-mahmoud.github.io/adapttable/og/cell-editing.png
slug: v2/cell-editing
---

▶ **Try it live:** [open a Mantine starter in StackBlitz](https://stackblitz.com/github/orwa-mahmoud/adapttable/tree/55da84f73ec078c0a075b812710f1619ec300875/starters/mantine?file=src%2FApp.tsx) — this page's feature is already wired in `src/App.tsx` (`editable` columns + `onCellEdit`); edit it in the browser, no install. [Other UI kits →](/adapttable/v2/getting-started/#try-it-in-stackblitz)
...
For an in-memory table, [`insertRow`, `removeRow` and `applyRowPatches`](/adapttable/v2/api/)
do the list work.

## Reordering rows

Changing which row sits where is the same one-way write: pass `onRowReorder`
and a grip appears. See [row reordering](/adapttable/v2/row-reordering/).
Pass `onPinnedRowIdsChange` and pin actions appear. See [row pinning](/adapttable/v2/row-pinning/).
...
helper is how a [realtime feed](/adapttable/v2/realtime/) lands:
...
* **Untouched rows keep their object identity.** React reconciles them as
  unchanged, and per-row memos — a [computed column](/adapttable/v2/columns/)'s cache, a
