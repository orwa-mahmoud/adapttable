---
title: "AdaptTable API reference — DataTable, columns, hooks (v1)"
...
head:
  ...
  - tag: script
    ...
    content: '{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"AdaptTable","item":"https://adapttable.orwamahmoud.com/"},{"@type":"ListItem","position":2,"name":"AdaptTable
      API reference — DataTable, columns,
      hooks","item":"https://adapttable.orwamahmoud.com/v1/react/api/"}]}'
  - tag: meta
    attrs:
      property: og:image
...
[URL-synced state](/v1/react/url-state/).
...
| `classNames`    | `DataTableClassNames`                           | —           | mantine, chakra, radix, base-ui, unstyled, shadcn | Per-part class overrides — five parts on Mantine/Chakra/Radix/Base UI (`root`/`toolbar`/`table`/`card`/`footer`), every part on unstyled and shadcn (shadcn merges yours over its preset). |
...
| `size`          | kit-specific union                              | —           | mui, chakra, antd, radix, base-ui                 | Explicit kit table size, overriding the size derived from `density` (chakra default `"md"`; radix/base-ui `"2"`, compact `"1"`).                                                           |
...
| `emptyState`    | `ReactNode`                                     | —           | unstyled, shadcn                                  | Empty-state node override (`slots.empty` alias wins when both are set).                                                                                                                    |
| `loadingState`  | `ReactNode`                                     | —           | unstyled, shadcn                                  | Loading-state node override (`slots.skeleton` alias wins when both are set).                                                                                                               |
...
* `useBackendData<TRow, TParams, TPage>(options): TableSource<TRow>` — wraps
  your `useInfiniteQuery`-based hook (`usePaginatedQuery`) into the same
  contract.
...
* `useRowExpansion(): RowExpansionState` — multi-open row expansion keyed by
  row id.
* `useCellEditing(): CellEditingState` — active-cell and draft state for
  inline editing (Enter / Escape / Tab flow).
* `useGroupCollapse(controlled?): GroupCollapseState` — collapse state for
  row groups (ephemeral; groups start expanded).
...
| `RowAction<TRow>` / `BulkAction`                                    | Action definitions with `disabledReason`, `isHidden` (row actions), optional `confirm` wiring; a bulk `onClick` receives `(ids, { allMatching, total })`. |
...
* `groupBy` or `exportCsv` `scope: "all"` on a server-paginated source (grouping is ignored; the current page is exported).

## Other packages

* `@adapttable/i18n` — `getLabels(locale)`, `getDirection(locale)`,
  `isRtlLocale(locale)`, `hasLocale(locale)`, `primarySubtag(locale)`,
  `RTL_LANGUAGES`, `locales` and the seventeen presets (`en`, `ar`, `de`,
  `es`, `fa`, `fr`, `he`, `hi`, `it`, `ja`, `ko`, `pt`, `ru`, `tr`, `ur`,
  `zh`, `zhTW`) — see [i18n & RTL](/v1/react/i18n-rtl/).
