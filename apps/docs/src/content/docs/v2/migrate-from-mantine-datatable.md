---
title: "Migrate from mantine-datatable — more built-in (v2)"
description: "@adapttable/mantine renders the same Mantine primitives, so the
  look barely changes — what changes is how much you wire by hand."
head:
  - tag: meta
    attrs:
      name: "robots"
  ...
  - tag: script
    ...
    content: '{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"AdaptTable","item":"https://orwa-mahmoud.github.io/adapttable/"},{"@type":"ListItem","position":2,"name":"Migrate
      from mantine-datatable — more
      built-in","item":"https://orwa-mahmoud.github.io/adapttable/v2/migrate-from-mantine-datatable/"}]}'
  - tag: meta
    attrs:
      property: og:image
  ...
  - tag: meta
    attrs:
      name: twitter:image
      content: https://orwa-mahmoud.github.io/adapttable/og/migrate-from-mantine-datatable.png
slug: v2/migrate-from-mantine-datatable
---
...
* **URL-synced, shareable, reload-safe state** — mantine-datatable keeps state
  in React only. See [URL state](/adapttable/v2/url-state/).
...
* **Saved views, an automatic mobile card layout, and opt-in
  ...
  [saved views](/adapttable/v2/saved-views/) and [virtualization](/adapttable/v2/virtualization/).
...
| `footer`                    | `summaryRow` (+ column `renderFooter`)              | One table-level function maps the page's rows to per-column footer cells; `renderFooter` replaces one column's cell. |
...
* **Stop sorting and slicing `records`.** On the frontend tier, hand the full
  ...
  table does it. For a server API, use the [server tier](/adapttable/v2/data-tiers/)
...
* [Getting started](/adapttable/v2/getting-started/) — install, providers, first table.
* [Filtering](/adapttable/v2/filtering/) · [Data tiers](/adapttable/v2/data-tiers/) ·
  [Column management](/adapttable/v2/column-management/) · [Saved views](/adapttable/v2/saved-views/).
