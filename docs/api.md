...
beside each speaker — pass an image, initials or a kit's own Avatar for
`assistant`, `user` or both; either side left out keeps the built-in glyph. The view types are `TableAssistantView`,
