...
the [advanced filter tree](/v2/react/filter-tree/).
...
* **Count filters** (the numeric operator + value pair, committed stable in
  [Versioning & stability](/v2/versioning/)): `COUNT_OPERATORS` is the operator
...
* **Operators**: `TEXT_OPS` / `NUMBER_OPS` / `DATE_OPS` are the stable URL
  ...
  Clear all drops `ft`. See [filter-tree](/v2/react/filter-tree/).
