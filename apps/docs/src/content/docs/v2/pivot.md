---
title: "React pivot table — rows, columns and measures (v2)"
description: Turn a React data table into a pivot table — row and column
  dimensions with aggregated measures, in Mantine, MUI, Chakra, Ant and more.
head:
  - tag: meta
    attrs:
      name: "robots"
  ...
  - tag: script
    ...
    content: '{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"AdaptTable","item":"https://orwa-mahmoud.github.io/adapttable/"},{"@type":"ListItem","position":2,"name":"React
      pivot table — rows, columns and
      measures","item":"https://orwa-mahmoud.github.io/adapttable/v2/pivot/"}]}'
  - tag: meta
    attrs:
      property: og:image
      content: https://orwa-mahmoud.github.io/adapttable/og/pivot.png
  - tag: meta
    attrs:
      name: twitter:image
      content: https://orwa-mahmoud.github.io/adapttable/og/pivot.png
...
const { rows, ...model } = pivotTableModel(result, { fields, labels });

<DataTable {...model} data={rows} />;
```
...
| `rows`                  | The table's `data`, keyed by the engine's own line key  |
...
`agg` may also be a name registered on the table; pass those as
`pivot(rows, config, { aggregators })`, a `ReadonlyMap<string, Aggregator>`.
`format(value, measure)` shapes each computed cell for display.
...
Drop it into a [side panel](/adapttable/v2/customization/) as that panel's content, and
...
the leaf ordering from the paths the server named, so the result is the *same*
...
All four are also on [`@adapttable/core/query`](/adapttable/v2/server-queries/#decoding-a-parameter-yourself),
...
Related: [row grouping](/adapttable/v2/row-grouping/) ·
[aggregation](/adapttable/v2/row-grouping/#aggregate-without-writing-the-maths) · [API reference](/adapttable/v2/api/)
