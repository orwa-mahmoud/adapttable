...
head:
  ...
  - tag: script
    ...
    content: '{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"AdaptTable","item":"https://adapttable.orwamahmoud.com/"},{"@type":"ListItem","position":2,"name":"Migrate
      ...
      MIT","item":"https://adapttable.orwamahmoud.com/v2/react/migrate-from-ag-grid/"}]}'
  - tag: meta
    attrs:
      property: og:image
  ...
  - tag: meta
    attrs:
      name: twitter:image
      content: https://adapttable.orwamahmoud.com/og/migrate-from-ag-grid.png
slug: v2/react/migrate-from-ag-grid
---
...
* **The analytics features are parts, not a mode.** [Pivoting](/v2/react/pivot/) is
  an engine plus its own panel; [nested grouping](/v2/react/row-grouping/) is a list
  of keys in `groupBy`; [tree data](/v2/react/tree-data/) is `getChildren` or
  `getParentId`; range selection, the fill handle and clipboard operations all
  need `cellNavigation` armed (see
  [Cell navigation](/v2/react/cell-navigation/)). Each works — none is the single
...
* **Cell editing is opt-in.** Map AG Grid `editable` columns to
  ...
  [Inline cell editing](/v2/react/cell-editing/)). `rowEditing` + `onRowEdit` edits a whole row
...
* **Row virtualization is opt-in** (`virtualize`) rather than always-on —
  ...
  [virtualization docs](/v2/react/virtualization/).
...
* [Comparison](/v2/react/comparison/) — the honest feature table, including where
  AG Grid wins.
