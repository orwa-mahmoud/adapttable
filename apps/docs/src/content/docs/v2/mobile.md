---
title: "Responsive React table — mobile card layout (v2)"
description: A React table that becomes a card list on phones automatically —
  same filters, search, selection and URL state. Tunable per column, no second
  layout to build.
head:
  - tag: meta
    attrs:
  ...
  - tag: script
    ...
    content: '{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"AdaptTable","item":"https://orwa-mahmoud.github.io/adapttable/"},{"@type":"ListItem","position":2,"name":"Responsive
      React table — mobile card
      layout","item":"https://orwa-mahmoud.github.io/adapttable/v2/mobile/"}]}'
  - tag: meta
    attrs:
      property: og:image
      content: https://orwa-mahmoud.github.io/adapttable/og/mobile.png
  - tag: meta
    attrs:
      name: twitter:image
      content: https://orwa-mahmoud.github.io/adapttable/og/mobile.png
...
<a href="https://orwa-mahmoud.github.io/adapttable/demo/mantine/mobile-cards/">
  <img src="https://orwa-mahmoud.github.io/adapttable/media/features/mobile.gif" alt="The same AdaptTable data table as phone cards — scrolling the card list, then switching to the desktop table" width="640" height="392" style="width:100%;height:auto;aspect-ratio:640/392;border-radius:8px" />
</a>
...
* **`hideOnMobile`** (per column) — drop a column from cards entirely. It
  always wins, including over `mobileIdentityColumns`.
* **`hideOnDesktop`** (per column) — the reverse: a field that appears only on
  the cards.
...
* **`rowClassName`** applies to desktop rows and mobile cards alike, and the
  [class-hook / `data-adapttable-part` surface](/adapttable/v2/customization/) names the
  card regions (class hooks `cardDetail`, `groupCard`, `summaryCard`; parts
  `card-detail`, `group-card`, `summary-card`) for styling.
...
* **Grouping** renders group header blocks between cards, with the same
  collapse behavior as desktop — see [row grouping](/adapttable/v2/row-grouping/).
* **Virtualization** windows the card list the same way it windows rows:
  ...
  see [virtualization](/adapttable/v2/virtualization/).
...
* **Every adapter ships it**: Mantine, MUI, Chakra, Ant Design, Radix,
  ...
  the [comparison table](/adapttable/v2/comparison/) tracks it as a built-in across
  the board.
...
Related: [Getting started](/adapttable/v2/getting-started/) ·
[API reference](/adapttable/v2/api/) ·
