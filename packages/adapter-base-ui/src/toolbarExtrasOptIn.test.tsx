/**
 * "Omitting a prop renders nothing" — proved for Base UI's toolbar extras.
 *
...
describe("Base UI toolbar extras are opt-in", () => {
  it("draws no undo/redo without BOTH handlers", () => {
    const only = renderBaseUi(
    ...
    const neither = renderBaseUi(<UndoRedoButtons {...base} />);
  ...
  it("draws no export button without a handler", () => {
    const { container } = renderBaseUi(<ExportCsvButton {...base} />);
