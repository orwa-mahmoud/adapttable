---
title: "React table inline cell editing — onCellEdit (v2)"
...
head:
  ...
  - tag: script
    ...
    content: '{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"AdaptTable","item":"https://adapttable.orwamahmoud.com/"},{"@type":"ListItem","position":2,"name":"React
      ...
      onCellEdit","item":"https://adapttable.orwamahmoud.com/v2/react/cell-editing/"}]}'
  - tag: meta
    attrs:
      property: og:image
      content: https://adapttable.orwamahmoud.com/og/cell-editing.png
  - tag: meta
    attrs:
      name: twitter:image
      content: https://adapttable.orwamahmoud.com/og/cell-editing.png
slug: v2/react/cell-editing
---

▶ **Try it live:** [open a Mantine starter in StackBlitz](https://stackblitz.com/github/orwa-mahmoud/adapttable/tree/55da84f73ec078c0a075b812710f1619ec300875/starters/mantine?file=src%2FApp.tsx) — this page's feature is already wired in `src/App.tsx` (`editable` columns + `onCellEdit`); edit it in the browser, no install. [Other UI kits →](/v2/react/getting-started/#try-it-in-stackblitz)
...
* **Per-column.** `editable` is `true`, `false`, or `(row) => boolean`. The
  editor defaults to `"text"`; use `"number"`, `{ type: "select", options }`
  or any other entry in [the editor set](#the-editor-set).
...
* **Persistence stays with the host.** The table never writes to a row; it
  reports the change and renders the save state you return.

## Options

| Prop / field  | Type                                                                  | Default  | Description                                                                            |
| ------------- | --------------------------------------------------------------------- | -------- | -------------------------------------------------------------------------------------- |
| `onCellEdit`  | `(row: TRow, key: string, nextValue: unknown) => unknown`             | —        | Change channel; its presence enables editing. Return a promise to show the save state. |
| `editable`    | `boolean \| ((row: TRow) => boolean)`                                 | —        | Whether this column can open an editor (still requires `onCellEdit`).                  |
| `editor`      | `CellEditor`                                                          | `"text"` | Widget for the active cell — see [the editor set](#the-editor-set).                    |
...
| `validateRow` | `(row) => string \| Record<string,string> \| undefined \| Promise<…>` | —        | Table rule over the row the edit would produce (cross-field).                          |
...
box, unstyled and shadcn render a native `<select multiple>`, and Radix Themes
and Base UI — whose select holds one value by construction — show a group of
their own checkboxes. That group is `MultiSelectEditorChrome`, core's structure
with a `Checkbox` slot, so both kits share one accessible group and neither of
them ships hand-written markup.
...
*means* is yours: which fields carry over, which reset, what id it gets.

For an in-memory table, [`insertRow`, `removeRow` and `applyRowPatches`](/v2/react/api/)
do the list work.

## Reordering rows

Changing which row sits where is the same one-way write: pass `onRowReorder`
and a grip appears. See [row reordering](/v2/react/row-reordering/).
Pass `onPinnedRowIdsChange` and pin actions appear. See [row pinning](/v2/react/row-pinning/).
...
| `EditValidationState`                                                                                                    | Validation state: which cells carry a message, which are still checking, and the `check` that gates one commit. |
...
helper is how a [realtime feed](/v2/react/realtime/) lands:
...
* **Untouched rows keep their object identity.** React reconciles them as
  unchanged, and per-row memos — a [computed column](/v2/react/columns/)'s cache, a
...
`undoRedoButtons` adds Undo and Redo buttons to the toolbar. They render only
when `editHistory` is set, and disable rather than disappear when there is
