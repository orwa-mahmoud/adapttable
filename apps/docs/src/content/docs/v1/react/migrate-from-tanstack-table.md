...
head:
  ...
  - tag: script
    ...
    content: '{"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"AdaptTable","item":"https://adapttable.orwamahmoud.com/"},{"@type":"ListItem","position":2,"name":"TanStack
      Table alternative — headless + ready UI
      kits","item":"https://adapttable.orwamahmoud.com/v1/react/migrate-from-tanstack-table/"}]}'
  - tag: meta
    attrs:
      property: og:image
  ...
  - tag: meta
    attrs:
      name: twitter:image
      content: https://adapttable.orwamahmoud.com/og/migrate-from-tanstack-table.png
slug: v1/react/migrate-from-tanstack-table
---
...
* **A filter UI.** TanStack gives you `columnFilters` state and
  ...
  declaration. See [filtering](/v1/react/filtering/).
...
* **URL-synced state.** TanStack has no URL API; you serialize
  ...
  [URL state](/v1/react/url-state/).
* **Saved views** — no concept in TanStack. See [saved views](/v1/react/saved-views/).
...
| `manualSorting` / `manualFiltering` / `manualPagination`              | `onQueryChange` (or `source` via `useBackendData`)                      | One consolidated server query. See [data tiers](/v1/data-tiers/). |
...
* [Concepts](/v1/concepts/) — the headless engine and `TableSource`.
* [Getting started](/v1/react/getting-started/) · [Filtering](/v1/react/filtering/) ·
  [Data tiers](/v1/data-tiers/) · [API reference](/v1/react/api/).
