 ...
 * from Chakra UI, and that the structure the chrome owns arrives intact.
 */
...
describe("TableAssistant", () => {
  ...
  it("draws its controls with Chakra UI's own components", () => {
    mount(
      <TableAssistant
        assistant={view()}
        labels={defaultLabels}
    ...
    expect(part("assistant-send")!.className).toContain("chakra-button");
  });
});
...
describe("the kit's own controls", () => {
  ...
  it("takes the sheet down when the host closes it", () => {
    // Chakra's Escape / interact-outside handlers are wired with native
    // pointer events jsdom cannot dispatch, which this repo already documents
    // for the filters drawer. The dismissal a reader reaches still arrives
    // here as the controlled `open` prop going false, so that is what is
    // driven — asserting a keypress jsdom cannot deliver would prove nothing.
